...
class Database{
    ...
    private let productListID = Expression<Int64>("listID")
...
//    LIST TABLE STUFF *************************************************************
    ...
    func getLists(forUserID userID: Int64) -> [(id: Int64, name: String)]? {
        var lists = [(id: Int64, name: String)]()
    ...
    private func createProductTable() {
        let createTable = productTable.create { table in
            ...
            table.column(productListID)
            table.column(productName)
            table.column(productCategory)
            table.column(productPrice)
            table.foreignKey(productListID, references: listTable, listID, delete: .cascade)
        }
    ...
    func addProduct(listID: Int64, productName: String, productCategory: String, productPrice: Double) -> Bool {
        do {
    ...
    // Method to retrieve products from a specific list
